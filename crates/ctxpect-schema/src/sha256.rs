//! SHA-256 (FIPS 180-4). No third-party crates: this workspace stays offline and
//! dependency-free, and content digests must be reproducible from source alone.
...
pub fn sha256_hex(data: &[u8]) -> String {
    ...
    // Padding: 0x80, zeroes, then the bit length as a big-endian u64.
...
mod tests {
    ...
    fn handles_block_boundaries() {
        // 55 / 56 / 63 / 64 / 119 / 120 bytes exercise every padding branch.
        for len in [55usize, 56, 63, 64, 119, 120, 1000] {
    ...
    fn handles_non_ascii() {
        assert_eq!(
            sha256_text("语义对齐"),
            sha256_hex("语义对齐".as_bytes()),
            "text hashing must be over UTF-8 bytes"
        );
    }
}
