...
| `ctxpect daemon start\|stop\|status` | WP-05 | 可选。`status` 以健康探测判断运行（`probe: reachable\|unreachable\|no_addr`）；`stop` 通过 store 内的本地 control token 请求 loopback shutdown，等所有权锁释放后才报告停止；缺 control 时拒绝，不按陈旧 PID 杀进程（见 [运维](operations.md)） |
