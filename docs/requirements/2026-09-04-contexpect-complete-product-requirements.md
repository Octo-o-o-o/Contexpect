...
- 大文件、历史和 archive 有明确大小/数量上限及跳过理由。当前 fs/collect 将 `MAX_READ_BYTES`（1 MiB）作为单文件保留正文的内存前缀上限，另有固定大小的流式读取/摘要缓冲；普通文件仍流式读取全文并计算全文件 SHA-256，超过前缀上限须标明保留正文已截断及实际长度，不得把前缀冒充完整正文。该阈值不是总 I/O、总 RSS 或扫描时长上限。历史导入、archive 解压与后续阶段的资源上限仍各自声明，不因本切片的全文摘要而放宽。
...
本节的“18 节外部用户验证”特指完整产品的正式用户验证；§18 允许的早期形成性反馈不属于这项先后约束，也不替代本门禁。
...
按 2026-09-05 用户授权的[统一交付安排](../plan/2026-09-05-contexpect-evidence-first-delivery.astra.md)，区分早期形成性反馈与全功能正式用户验证。

只读核心通过相应切片的安全、稳定性与可解释性内部验收后，可以开展小范围形成性反馈，观察用户能否解释纳入/排除、找到证据、理解 Unknown 并完成实际排查任务。可先用合成任务准备材料；招募、记录收集和参与者真实仓库使用按具体授权与同意进行。失败、误解和无价值情形也记录为研究结果，小样本反馈不作为代码正确率或全产品效果的证明。

早期反馈用于调整后段优先级和实现深度，不替代 F-01–F-18、WP-01–WP-12、§17 的全功能验收及下述正式验证；完整交付范围的减少另作明确合同修订。全功能正式用户验证仍发生在完整产品通过内部验收之后。
