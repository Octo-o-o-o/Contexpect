...
- 2026-09-11 终审复核与修复（四路 diff 审查 + 独立 gate 复跑后）：修复 ui-e2e 两红（sessions 断言改为引用 i18n `boolNo` 并加反向断言；820px 横向溢出根因为 `.f-firstseen` 的 12em 最小宽度，改为其仅在 ≥1280px 生效并给 findings 表加面板内横向滚动）；修复 cancel 用例时序假阳性、`.chain-glyph` 恒真空断言、delayed-bootstrap 固定等待改 `expect.poll`；UI 侧修复 data-undeclared 选择器永不命中、`<project>` 占位符改为 daemonHasProject 布尔（unset 时禁用检查并如实提示）、bootstrap 成功清除残留错误、死 CSS 清理、`--text-faint`/nav-group-label/brand-sub/shell-foot 对比度提到 ≥4.5:1、coverage 图例补 unknown 色点、空 severity 不再渲染空框、窄屏忽略 ⌘K、page-contract 三页补 `?receipt_id=` 说明；daemon 侧修复 `diagnosis_basis` 恒常量（现按事实输出 null / `receipt-only` / `receipt-and-current-project-scan`，schema 同步放宽）、当前 Receipt 被 tombstone 时 status 回退 latest-matching 而非 400、失配错误码按原因区分（新增 `api.receipt_coordinate_drift`）、codex_home 比较的 fail-open、suppression 不能再抑制「抑制无效」告警本身、DSH header-only 无尾换行按 torn 处理；`snapshot_paths` 跳过非 UTF-8 文件并提示（不再被 .DS_Store 整盘打挂）。记录在案未修：`is_js_shortest_form` 尾数规格化假阳性（纵深防御边界，已在函数注释说明）。最终复验：16 条 required gate 与可选 ui-e2e（12 passed / 0 failed）全部退出码 0；Rust workspace 含新增 3 测试全绿，validator-negative 149 OK。本批修复未 commit。


2026-09-12 提交前接续：上述“记录在案未修”的数字尾数规格化假阳性已补拒绝规则与 schema/importer 正反例，并额外拒绝浮点溢出、下溢和错误指数阈值；仍只声明数字形状过滤，不声明实现任意 IEEE-754 数字的完整 shortest-round-trip canonicalizer。比较页的 Receipt 选择与请求代次绑定、列表离线重试及跨坐标提示亦已补齐，详见 [交接包价值裁决](2026-09-12-handoff-value-implementation.md)。
